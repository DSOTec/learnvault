...
export async function scanFileWithClamAV(
	...
	return new Promise((resolve) => {
		...
		client.on("end", () => {
			clearTimeout(timer)
			const isClean =
				responseData.includes("stream: OK") || responseData.includes("OK")
			...
			if (isClean && !isVirus) {
				...
				resolve({
					...
					details:
						responseData.trim() ||
						"ClamAV returned a virus warning or parsing error.",
				})
			}
		})
...
export async function stripImageMetadata(
	...
	try {
		...
		throw new Error(
			`Failed to strip image metadata: ${error instanceof Error ? error.message : String(error)}`,
		)
	}
}
...
// ---------------------------------------------------------------------------
export async function handleSecureUploadRoute(
	req: Request,
	res: Response,
): Promise<void> {
	...
	uploadMiddleware(req, res, async (err: any) => {
		...
		try {
			...
			const sanitizedBuffer = await stripImageMetadata(
				buffer,
				magicScan.detectedMime,
			)
			...
			res.status(400).json({
				...
				message:
					sanitizeError instanceof Error
						...
						: "Failed to sanitize image metadata.",
