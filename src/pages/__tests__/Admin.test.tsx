...
import { useAdminMilestones, useAdminStats } from "../../hooks/useAdmin"
...
describe("Admin Page", () => {
	...
	it("renders pending milestones for admins", async () => {
		...
		expect(
			within(firstRow).getByRole("button", { name: /approve/i }),
		).toBeVisible()
		expect(
			within(firstRow).getByRole("button", { name: /reject/i }),
		).toBeVisible()
	})
	...
	it("removes milestone from list after successful approval", async () => {
		...
		await waitFor(() => {
			expect(
				screen.queryByRole("row", { name: /glearner1/i }),
