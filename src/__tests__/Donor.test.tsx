...
vi.mock("../hooks/useContractIds", () => ({
	useContractIds: () => ({
		scholarshipTreasury:
			"CC32NKWO6V5UCDV674HBE32N4C3EAWBVO5M4U6Y74E2ST7B3TREASURY",
	}),
}))
...
describe("Donor Page / Deposit Suite", () => {
	...
	it("renders Connect Wallet CTA when disconnected", () => {
		...
		const submitBtn = screen.getByRole("button", {
			name: /Connect Wallet to Deposit/i,
		})
	...
	it("submits deposit, updates balance on success, and displays success banner", async () => {
		...
		await waitFor(() => {
			...
			expect(mockDepositMethod).toHaveBeenCalledWith(
				"100.00",
				mockSignTransaction,
			)
