...
async function fetchCredentialFromSoroban(
	nftId: string,
	...
	try {
		...
		return {
			scholarAddress:
				"GDQP2CNOEF32NKWO6V5UCDV674HBE32N4C3EAWBVO5M4U6Y74E2ST7B3",
	...
	try {
		...
		if (!credential) {
			return {
				...
				description:
					"The requested credential does not exist or has not been verified on the Stellar ledger.",
			}
		}
		return {
		...
		return {
			...
			description:
				"Unable to verify credential at this time due to an RPC communication issue.",
		}
	}
}
			...
			<div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6">
				<div className="max-w-md w-full bg-slate-900 border border-red-500/20 rounded-2xl p-6 text-center shadow-xl">
					...
					<p className="text-sm text-slate-400 mb-6">
						We are currently unable to query the Soroban network to verify this
						credential. Please try again later.
					</p>
					...
					<Link
						href="/"
						className="inline-block bg-slate-800 hover:bg-slate-700 text-sm font-medium px-4 py-2 rounded-lg transition-colors"
					>
						Return to Dashboard
					</Link>
				</div>
			...
			<div className="min-h-screen bg-slate-950 text-slate-100 flex flex-col items-center justify-center p-6">
				<div className="max-w-md w-full bg-slate-900 border border-slate-800 rounded-2xl p-8 text-center shadow-xl">
					...
					<p className="text-sm text-slate-400 mb-6">
						...
						<span className="font-mono text-slate-300">{nftId}</span> could not
						be verified on the Stellar ledger.
					</p>
					<Link
						href="/"
						className="inline-block bg-slate-800 hover:bg-slate-700 text-sm font-medium px-4 py-2 rounded-lg transition-colors"
					...
					<div className="p-6 md:p-8 flex flex-col justify-between">
							...
							<div className="space-y-4">
								...
								<div>
									...
									<p className="text-sm font-medium text-slate-300">
										{new Date(credential.completionDate).toLocaleDateString(
											undefined,
											{
												year: "numeric",
												month: "long",
						...
						<div className="mt-8 pt-6 border-t border-slate-800 flex flex-col gap-3">
							<div className="flex items-center gap-2 text-xs text-slate-400">
								...
								<span>
									Tamper-proof record verified via Soroban Smart Contract.
								</span>
							</div>
						</div>
					</div>
